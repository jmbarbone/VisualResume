...
#' @param font.family character. An optional google font family. Run \code{sysfonts::font_families_google()} to see all of them. Only use when creating plot in a plotting device such as pdf()
...
#' @importFrom sysfonts font_add_google font_families_google
#'
#' @export
#'
#' @details
...
VisualResume <- function(titles.left      = c("Main Title", "Sub-title", "Sub-Sub-title"),
  ...
  ## Colors and Fonts
  {
    ...
    if(font.family %in% sysfonts::font_families_google()) {

      google.font <- TRUE
      sysfonts::font_families_google(font.family, font.family)
  ...
  # ----------------
  {
    ...
    # Top separation
    points(
      x   = seq(year.min - 1, year.max + 1, length.out = 50),
      y   = rep(100, 50),
      pch = 16,
      col = c(gray(.7)),
      cex = c(1, .7))
    ...
    # ...
    points(
      x   = seq(year.min - .75, year.min - .25, length.out = 3),
      y   = rep(50, 3),
      cex = c(.5, .75, 1.75),
      pch = 21,
      bg  = "white"
    ...
    points(
      x   = seq(year.max +.25, year.max + .75, length.out = 3),
      y   = rep(50, 3),
      cex = rev(c(.5, .75, 1.75)),
      pch = 21,
      bg  = "white"
    ...
    segments(
      x0  = year.seq,
      y0  = rep(bottom.y0, length(year.seq)),
      x1  = year.seq,
      y1  = rep(top.y0, length(year.seq)),
      col = "lightgray"
    )

    rect(
      xleft   = year.seq[-length(year.seq)],
      ybottom = rep(bottom.y0, length(year.seq)),
      xright  = year.seq[2:length(year.seq)],
      ytop    = rep(top.y0, length(year.seq)),
      col     = c("white"),
      border  = gray(.5)
    )
    ...
    #  --------
    {
      ...
      for (i in 1:nrow(timeline)) {
        ...
        points.conflicting <- sum(abs(timeline$point.x[-i] - point.x) < .1 &
                                    abs(timeline$point.y[-i] - point.y) < .1,
                                  na.rm = TRUE)
      ...
      for(i in 1:nrow(timeline)) {
        ...
        if(is.na(timeline$label.x[i])) {
          if(method == 1) {
            ...
            right.conflicting.l <-
              (timeline$start[-i] >= timeline$start[i]) &
              (timeline$start[-i] <= timeline$start[i] + scare.dist) &
              (timeline$side[-i] == side.i)
            ...
            if(any(right.conflicting.l) == FALSE & any(left.conflicting.l) == FALSE) {
            ...
            if(i > 1) {
              label.conflict.l <-
                (timeline$point.x[-i] - timeline$point.x[i]) > -scare.dist &
            ...
            if(i > 1) {
              for(j in 1:(i - 1)) {
                if(timeline$side[j] == timeline$side[i]) {
                  ...
                  y.dev[x.dev.l] <- y.dev[x.dev.l] +
                    as.numeric(abs(other.y.loc - location.mtx$y[x.dev.l]) < 5) * 10
                }
              }
    ...
    for(i in 1:nrow(timeline)) {
      ...
      # Points
      points(x   = timeline$point.x[i],
             y   = timeline$point.y[i],
             pch = 21,
             cex = 2,
             col = "black",
             bg  = "white")
      ...
      # Main line
      segments(x0  = timeline$point.x[i],
               y0  = timeline$point.y[i],
               x1  = timeline$label.x[i],
               y1  = timeline$label.y[i],
               lty = 2)

      # Elbow line
      segments(x0  = timeline$label.x[i],
               y0  = timeline$label.y[i],
               x1  = timeline$label.x[i] + timeline$elbow[i],
               y1  = timeline$label.y[i],
               lty = 2)

      # Main Text
      text(x      = timeline$label.x[i] + timeline$elbow[i],
           y      = timeline$label.y[i],
    ...
    # --------- Upper milestones
    text(x      = milestones$year,
         y      = rep(95, nrow(milestones)),
         labels = milestones$title,
         cex    = 2,
         adj    = 1,
         family = font.family,
         font   = 3)
    ...
    text(x      = milestones$year,
         y      = rep(90, nrow(milestones)),
         labels = milestones$sub,
         cex    = 1.5,
         adj    = 1,
         family = font.family,
         font   = 1)
  ...
  # ----------------
  {
    # --- Bottom left Interests
    {
      ...
      if(n.interests == 2) {
        locations <- data.frame(x = c(25, 75),
                                y = c(35, 55) / 2.5)
      ...
      if(n.interests == 3) {
        locations <- data.frame(x = c(15, 50, 85),
                                y = c(20, 75, 40) / 2.5)
      ...
      if(n.interests == 4) {
        locations <- data.frame(x = seq(10, 90, length.out = 4),
                                y = c(20, 75, 20, 80) / 2.5)
    ...
    # ---- Bottom right Events
    {
      ...
      text(
        x      = rep(.1, n.events),
        y      = seq(.9, .0, length.out = n.events),
        labels = events[1:nrow(events) %in% events.selected,2],
        adj    = 0,
        cex    = events.cex,
        family = font.family
      )
